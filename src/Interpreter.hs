...
module Interpreter (getInterpreter) where
...
getInterpreter = do s <- readFile "primit.scm"
                    intp <- getIntp
                    runExceptT (intp s)
                    pure intp
...
evalAll env s = runReaderT (parseAll' s >>= interpretAll) env  >>= (lift . unRefs)
...
define k v = asks head >>=  (liftIO . runMaybeT . def) >>= maybe (throw ("duplicated definition :" ++ k)) pure
...
interpret (List x) = case x of
    ...
    where interpretBody []  = throw "empty function body"
